...
除了 `services.moviepilot.settings` 这种直接转成后端环境变量的配置外，模块还可以通过 MoviePilot 自身 API 声明式同步 UI 数据。

当前支持的声明式对象：

- 下载器：`services.moviepilot.downloaders`
- 整理目录：`services.moviepilot.directories`
- 媒体服务器：`services.moviepilot.mediaServers`
- 存储：`services.moviepilot.storages`
- 站点认证：`services.moviepilot.siteAuth`
- 站点：`services.moviepilot.sites`
- 订阅：`services.moviepilot.subscriptions`
- 索引站点选择：`services.moviepilot.indexerSites`
- RSS 站点选择：`services.moviepilot.rssSites`
- 已安装插件：`services.moviepilot.installedPlugins`
- 插件配置：`services.moviepilot.pluginConfigs`
- 插件分组：`services.moviepilot.pluginFolders`

这套同步机制有几个关键语义：

- 需要 `API_TOKEN`
  只要用了上面任一声明式对象，或希望声明式同步管理员密码，就必须通过 `services.moviepilot.settings.API_TOKEN` 或 `services.moviepilot.environmentFile` 提供 `API_TOKEN`
- 不直接写数据库
  模块通过 MoviePilot API 同步配置，不依赖 SQLite/PostgreSQL 的底层表结构
- UI 不是最终真源
  对于你已经放进 Nix 的对象，UI 中的修改会在下次 `switch`、服务重启或对应 seed service 重跑时被声明式配置重新收敛
- 管理员密码同步是可选的
  只有在你设置了 `SUPERUSER_PASSWORD` 时，模块才会额外通过 API 对齐管理员密码

敏感值注入方式按对象类型区分：

- `downloaders[*].configFromEnvironment`
- `mediaServers[*].configFromEnvironment`
- `storages[*].configFromEnvironment`
- `sites[*].fromEnvironment`
- `siteAuth.paramsFromEnvironment`
- `pluginConfigs.<plugin>.fromEnvironment`
...
```nix
{
  services.moviepilot = {
    enable = true;
    # API_TOKEN、QBITTORRENT_PASSWORD、JELLYFIN_API_KEY 等敏感值放这里
    environmentFile = "/run/secrets/moviepilot.env";
    ...
    downloaders = [
      {
        ...
        configFromEnvironment.password = "QBITTORRENT_PASSWORD";
        pathMapping = [
          {
    ...
    mediaServers = [
      {
        name = "Jellyfin";
        type = "jellyfin";
        config.host = "http://127.0.0.1:8096";
        configFromEnvironment.apikey = "JELLYFIN_API_KEY";
    ...
    storages = [
      ...
      }
      {
        ...
        configFromEnvironment.password = "OPENLIST_PASSWORD";
      }
    ];

    siteAuth = {
      site = "iyuu";
      paramsFromEnvironment.token = "IYUU_SITE_TOKEN";
    };
  };
}
```
...
- `services.moviepilot.user`
- `services.moviepilot.group`
...
- `services.moviepilot.sandboxRoot`
  后端 systemd 沙箱允许访问的根目录
  你声明的整理目录、下载目录、媒体库路径都必须落在这个范围内
- `services.moviepilot.managedDirectories`
  额外需要由 `systemd-tmpfiles` 创建并校正权限的目录
- `services.moviepilot.downloaders`
  声明式写入下载器；敏感字段走 `configFromEnvironment`
- `services.moviepilot.directories`
  声明式写入整理目录、监控方式和媒体库路径
- `services.moviepilot.mediaServers`
  声明式写入媒体服务器；敏感字段走 `configFromEnvironment`
- `services.moviepilot.storages`
  声明式写入本地或远端存储；敏感字段走 `configFromEnvironment`
- `services.moviepilot.siteAuth`
  声明式写入站点认证方案，例如 IYUU；敏感字段走 `paramsFromEnvironment`
- `services.moviepilot.sites`
  声明式写入站点；敏感字段走 `fromEnvironment`
- `services.moviepilot.subscriptions`
  声明式写入订阅；`sites` 字段写 domain，模块会自动解析成站点 ID
- `services.moviepilot.indexerSites`
  声明式写入“站点索引”启用站点列表
- `services.moviepilot.rssSites`
  声明式写入 RSS 启用站点列表
